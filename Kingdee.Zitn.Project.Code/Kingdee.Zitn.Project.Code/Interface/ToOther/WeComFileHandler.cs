...
{
    /// <summary>
    /// 企业微信文件消息发送HTTP处理程序 - 无状态，外部系统可直接调用
    /// </summary>
    public class WeComFileHandler : IHttpHandler
    {
        private static readonly CustomLog.LogWriter _log = CustomLog.For("企业微信文件消息HTTP接口");
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                _log?.WriteLog($"收到文件接口请求，Content-Type={contentType}，Body长度={body?.Length ?? 0}，Body前100字符={body?.Substring(0, Math.Min(body.Length, 100)) ?? "空"}");
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        string uploadError;
                                        mediaId = SendMsg.UploadMedia(base64Data, "file", out uploadError, fileName);
                                        if (string.IsNullOrWhiteSpace(mediaId))
                                        {
                                            returnCode = "4000";
                                            returnMsg = "文件素材上传失败: " + uploadError;
                                        }
                                    }
                                    ...
                                    {
                                        // 6. 发送文件消息
                                        SendMsg.SendFile(mediaId, toUser);
                                        _log?.WriteLog($"企业微信文件消息发送成功: {toUserChinese} -> {toUser}");
