...
JOIN T_STK_STKTRANSFERINENTRY E ON H.FID = E.FID
...
WHERE H.FID = {fid}";
                    ...
                    for (int i = 0; i < entries.Count; i++)
                    {
                        ...
                        var qty = Convert.ToDecimal(entries[i]["FQTY"]);

                        if (qty <= 0)
                        {
                            log.WriteLog($"  EntryID={entryId} 物料{materialNo} 调拨数量为0，跳过");
                            continue;
                        }
...
ORDER BY FFHBZLB, FSCBZGG1, FCP";
                        ...
                        var updateEntrySql = $@"/*dialect*/UPDATE T_STK_STKTRANSFERINENTRY
