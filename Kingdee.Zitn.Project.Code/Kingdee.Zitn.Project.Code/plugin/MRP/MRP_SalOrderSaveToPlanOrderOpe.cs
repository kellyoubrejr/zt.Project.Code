...
{
    [Description("销售订单保存根据多投数量生成计划订单-服务"), HotUpdate]
    public class MRP_SalOrderSaveToPlanOrderOpe : AbstractOperationServicePlugIn
    {
        ...
        public override void OnPreparePropertys(PreparePropertysEventArgs e)
        {
            ...
            e.FieldKeys.Add("F_UNW_Integer_qtr");
            e.FieldKeys.Add("F_UNW_QTY_QTR");
            e.FieldKeys.Add("F_PAEZ_SJTCQTY");
        }
        ...
        public override void AfterExecuteOperationTransaction(AfterExecuteOperationTransaction e)
        {
            ...
            foreach (var selectedRow in e.SelectedRows)
            {
                ...
                if (!(entity?["SaleOrderEntry"] is DynamicObjectCollection entries)) continue;
            ...
            foreach (DynamicObject row in dt)
            {
                ...
                var billTypeId = row["BILLTYPEID"]?.ToString() ?? "";
                ...
                switch (billTypeId)
                {
                    ...
                    case BILLTYPE_PRE_CAST:
                        ...
                        qty = entryDataLookup.TryGetValue(entryId, out var aEnt)
                            ? Convert.ToDecimal(aEnt["F_UNW_QTY_QTR"] ?? 0)
                            : 0;
                        break;
                    case BILLTYPE_INTERNAL_TRIAL:
                    case BILLTYPE_DISPATCH_PRE_CAST:

                        qty = entryDataLookup.TryGetValue(entryId, out var bEnt)
                            ? Convert.ToDecimal(bEnt["F_PAEZ_SJTCQTY"] ?? 0)
                            : 0;
                        jhddType = "2";
                        break;
                    default:
                ...
                var formDtsl = formEnt?["F_UNW_QTY_QTR"];
                var formSjtc = formEnt?["F_PAEZ_SJTCQTY"];
                _log.WriteLog($"DEBUG: billTypeId={billTypeId}, SQL_DTSL={row["DTSL"]}, SQL_SJTCSL={row["SJTCSL"]}, Form_DTSL={formDtsl}, Form_SJTCSL={formSjtc}, final_qty={qty}");
                ...
                try
                {
                    ...
                    if (responseStatus != null)
                    {
                        ...
                        if (isSuccess)
                        {
                            ...
                            var upsql = $@"UPDATE T_SAL_ORDERENTRY SET F_UNW_Integer_qtr = {planId} WHERE FENTRYID = {entryId}";
        ...
        public override void BeginOperationTransaction(BeginOperationTransactionArgs e)
        {
            ...
            {
                if (!(dataEntity["SaleOrderEntry"] is DynamicObjectCollection entries)) continue;
                foreach (var entry in entries)
                {
                    var entId = Convert.ToInt64(entry["Id"]);
                    if (_entryPlanMap.TryGetValue(entId, out var planId))
                    {
                        entry["F_UNW_Integer_qtr"] = planId;
                    }
                }
            }
        }
