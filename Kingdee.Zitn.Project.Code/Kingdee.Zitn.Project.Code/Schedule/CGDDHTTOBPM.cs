...
{
    [Description("【跑批】：同步当日审核采购订单推送bpm合同信息")]
    [Kingdee.BOS.Util.HotUpdate]
    public class CGDDHTTOBPM : IScheduleService
    {
        private static readonly CustomLog.LogWriter _log = CustomLog.For("BPM采购合同计划");
        public void Run(BOS.Context ctx, BOS.Core.Schedule schedule)
        {
            PushToBpmHT(ctx);
        }

        private void PushToBpmHT(BOS.Context ctx)
        {
            int totalCount = 0, successCount = 0, skipCount = 0, failCount = 0;
            try
            {
                var qry = string.Format("/*dialect*/SELECT FTSSTATUS,FFHXX,FBILLNO FROM T_PUR_POORDER WHERE FDOCUMENTSTATUS= 'C' AND CAST(FAPPROVEDATE AS DATE) = CAST(GETDATE() AS DATE)");
                var dt = DBUtils.ExecuteDynamicObject(ctx, qry);
                _log.Section($"开始执行采购订单推送BPM合同定时任务，查询到 {dt?.Count ?? 0} 条今日审核订单");
                ...
                if (dt != null && dt.Count > 0)
                {
                    ...
                    {
                        var dt2 = dt[i];
                        string billno = dt2["FBILLNO"]?.ToString();

                        try
                        {
                            bool falg = GetJudgeHTFeild(ctx, billno);
                            if (falg)
                            {
                                _log.WriteLog($"[{i + 1}/{totalCount}] 单据 {billno} 已存在合同号，跳过");
                                WritePushStatus(billno, false, "采购订单已存在合同号，跳过推送", ctx);
                                skipCount++;
                                continue;
                            }

                            var requestList = GetPurchaseOrderData(billno, ctx);
                            ...
                            _log.WriteLog($"[{i + 1}/{totalCount}] 开始推送单据 {billno}");
                            LogEmptyFields(requestList);
                            bool success = CallPostApi(apiUrl, jsonBody, out string response);
                            WritePushStatus(billno, success, response, ctx);

                            if (success)
                            {
                                _log.WriteLog($"[{i + 1}/{totalCount}] 单据 {billno} 推送成功");
                                successCount++;
                            }
                            else
                            {
                                _log.WriteLog($"[{i + 1}/{totalCount}] 单据 {billno} 推送失败：{response}");
                                SendMsg.Send($"单据 {billno} 推送BPM合同失败：{response}");
                                failCount++;
                            }
                        }
                        catch (Exception ex)
                        {
                            _log.WriteLog($"[{i + 1}/{totalCount}] 单据 {billno} 处理异常：{ex.Message}");
                            SendMsg.Send($"单据 {billno} 推送BPM合同异常：{ex.Message}");
                            WritePushStatus(billno, false, ex.Message, ctx);
                            failCount++;
                        }
                    }
                }

                _log.Section($"定时任务执行完成：总数={totalCount}, 成功={successCount}, 跳过={skipCount}, 失败={failCount}");
            }
            catch (Exception ex)
            {
                _log.Error($"定时任务执行异常：{ex.Message}");
                _log.Error(ex);
                SendMsg.Send($"采购订单推送BPM合同定时任务执行异常：{ex.Message}");
            }
        }
        ...
        private bool CallPostApi(string url, string jsonData, out string responseText)
        {
            ...
            try
            {
                ...
                {
                    responseText = "接口无响应";
                    _log.WriteLog($"接口返回异常: 无响应");
                    return false;
                }

                int statusCode = (int)response.StatusCode;
                _log.WriteLog($"HTTP状态码: {statusCode}");
            ...
            catch (Exception ex)
            {
                ...
                _log.WriteLog($"接口调用异常: {ex.Message}");
                return false;
            }
        }
            ...
            for (int i = 0; i < requestList.Count; i++)
            {
                ...
                if (order.TryGetValue("pur", out var purObj) && purObj is Dictionary<string, object> pur)
                {
                    foreach (var kv in pur)
                    {
                        ...
                        if (isEmpty)
                        {
                            _log.WriteLog($"  [空] pur.{kv.Key} = {(val == null ? "null" : "\"\"")}");
                        }
                    }
                }
                ...
                if (order.TryGetValue("materials", out var matsObj) && matsObj is List<Dictionary<string, object>> materials)
                {
                    for (int m = 0; m < materials.Count; m++)
                    {
                        foreach (var kv in materials[m])
                        {
                            ...
                            if (isEmpty)
                            {
                                _log.WriteLog($"  [空] materials[{m}].{kv.Key} = {(val == null ? "null" : "\"\"")}");
                            }
                        }
                    }
                }
                ...
                if (order.TryGetValue("payment", out var payObj) && payObj is List<Dictionary<string, object>> payments)
                {
                    for (int p = 0; p < payments.Count; p++)
                    {
                        foreach (var kv in payments[p])
                        {
                            ...
                            if (isEmpty)
                            {
                                _log.WriteLog($"  [空] payment[{p}].{kv.Key} = {(val == null ? "null" : "\"\"")}");
                            }
                        }
                    }
                }
            }
            _log.WriteLog("--- 字段检查完毕 ---");
        }

        private List<Dictionary<string, object>> GetPurchaseOrderData(string poBillno, Context ctx)
        {
            ...
            DynamicObjectCollection POcol = DBUtils.ExecuteDynamicObject(ctx, poSql);
            DynamicObjectCollection PAYcol = DBUtils.ExecuteDynamicObject(ctx, paySql);
        ...
        private void WritePushStatus(string poBillno, bool success, string response, Context ctx)
        {
            try
            {
        ...
        private bool GetJudgeHTFeild(Context ctx, string poBillno)
        {
            ...
            DynamicObjectCollection dt = DBUtils.ExecuteDynamicObject(ctx, jsql);
